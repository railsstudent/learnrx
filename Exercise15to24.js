...
Let's repeat exercise 21, but this time lets use your new zip() function. For 
each video and bookmark pair, create a {videoId, bookmarkId} pair.
*/
var ex23 = function() {
	var videos = [
			{
...
screenshot is interesting or representative of the title as a whole. Notice that 
both the boxarts and interestingMoments arrays are located at the same depth in 
the tree. Retrieve the time of the middle interesting moment and the smallest 
box art url simultaneously with zip(). Return an {id, title, time, url} objec
t for each video.
*/
var ex24 = function() {
	var movieLists = [
	...
	return movieLists.
		concatMap(function(movieList) {
				return movieList.videos.concatMap(function(video) {
							var smallestBoxart = video.boxarts
								.reduce(function(acc, boxart) {
									return (acc.width * acc.height) < (boxart.width * boxart.height) ? acc : boxart;
								});
								
							var middleMoment = video.interestingMoments
								.filter(function(m) {
									return m.type === "Middle";
								});
							return Array.zip(middleMoment, smallestBoxart, function(l, r) {
								return { id: video.id, title: video.title, time: l.time, url: r.url};
							});	
				});
		});
}
